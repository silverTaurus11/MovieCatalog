...
@AndroidEntryPoint
class TvShowFragment: Fragment(), SortMoviesListener {
    companion object{
    ...
    private fun initTvShowListObserve(){
        ...
        tvShowViewModel.tvShows.observe(viewLifecycleOwner, Observer { tvShowData ->
            setUpObserver(tvShowData)
        })
    }

    private fun setUpObserver(tvShowData: Resource<PagedList<TvShowEntity>>){
        when(tvShowData){
            is Resource.Loading -> {
    ...
    private fun showDataList(data: PagedList<TvShowEntity>?){
    ...
    override fun doSortMovieByRating() {
        EspressoIdlingResource.increment()
        if(isDataListVisible()){
            tvShowViewModel.orderByRating().observe(viewLifecycleOwner, Observer { tvShowData ->
                setUpObserver(tvShowData)
            })
        }
    }
